    ...
    public void saveBeyondCapacity() {
        try {
            for (int i = storage.size(); i < STORAGE_CAPACITY; i++) {
                ...
                storage.save(Resumes.fillOut(StringValueOfIndex, "NAME-" + StringValueOfIndex));
            }
        } catch (StorageException e) {
            fail("Error: storage overflow within storage capacity");
        }
        // assertThrows(StorageException.class, () -> storage.save(new Resume("Name")));
        assertThrows(StorageException.class, () -> storage.save(Resumes.fillOut(String.valueOf(STORAGE_CAPACITY), "NAME-" + STORAGE_CAPACITY)));
    }
}
